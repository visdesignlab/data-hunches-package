...
var DataHunchIndicator = function (_a) {
    ...
    return ((0, jsx_runtime_1.jsxs)("g", { children: [inVisDH.map(function (d, i) {
                if (parseFloat(d.content) > verticalValueScale.domain()[0]) {
                    return (0, jsx_runtime_1.jsx)(OverAxisIndicator_1.default, { dataHunch: d }, d.id);
                }
                if (inVisDH.length > 3) {
                    return ((0, jsx_runtime_1.jsx)(StyledComponents_1.DHIndicatorRect, { onMouseOver: function () { store.setHighlightedDH(d.id); }, x: honrizontalBandScale(d.label) || 0, width: honrizontalBandScale.bandwidth(), y: calculateY(d, true) }, d.id));
                }
                else {
                    return (0, jsx_runtime_1.jsx)(SketchyBar_1.default, { xPos: (honrizontalBandScale(d.label) || 0) + (honrizontalBandScale.bandwidth() / inVisDH.length * i), yPos: calculateY(d, false), width: honrizontalBandScale.bandwidth() / inVisDH.length, height: calculateHeight(d) }, d.id);
                }
            }), offVisDH.map(function (d, i) {
                return ((0, jsx_runtime_1.jsx)(core_1.Tooltip, __assign({ title: d.reasoning }, { children: (0, jsx_runtime_1.jsx)(StyledComponents_1.DHIndicatorText, __assign({ x: (honrizontalBandScale(d.label) || 0) + 0.5 * honrizontalBandScale.bandwidth() + (2 * Constants_1.IndicatorSize + Constants_1.IndicatorSpace) * (i % 2 === 0 ? -1 : 1), y: store.svgHeight - Constants_1.margin.bottom + 25 + 2 * (Constants_1.IndicatorSize + Constants_1.IndicatorSpace) * Math.floor(i / 2), fontSize: d.type === 'exclusion' ? 'small' : 'large' }, { children: d.type === 'exclusion' ? 'x' : '*' }), d.id) }), d.id));
            })] }, void 0));
};
