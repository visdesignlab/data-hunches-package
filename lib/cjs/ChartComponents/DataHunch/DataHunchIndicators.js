...
var DataHunchIndicator = function (_a) {
    ...
    var calculateX = function (dataHunch, rangeCenter) {
        if (dataHunch.type === 'range') {
            ...
            if (rangeCenter) {
                ...
                return valueScale((0, d3_array_1.max)(parsedRange));
            }
        }
    ...
    var calculateWidth = function (dataHunch) {
        if (dataHunch.type === 'range') {
            ...
            return store.svgHeight - Constants_1.margin.bottom - valueScale(parseFloat(dataHunch.content));
        }
    };
    ...
    return ((0, jsx_runtime_1.jsxs)("g", { children: [inVisDH.map(function (d, i) {
                ...
                if (inVisDH.length > 3) {
                    return ((0, jsx_runtime_1.jsx)(DHIndicatorRect_1.default, { dataHunch: d, xPos: calculateX(d, true), height: bandScale.bandwidth(), yPos: bandScale(d.label) || 0 }, "".concat(d.id, "-dhindicatorRect")));
                }
                else {
                    return ((0, jsx_runtime_1.jsx)(SketchyBar_1.default, { dataHunch: d, xPos: calculateX(d, false), yPos: (bandScale(d.label) || 0) + bandScale.bandwidth() / inVisDH.length * i, width: calculateWidth(d), height: bandScale.bandwidth() / inVisDH.length }, "".concat(d.id, "-dhindicatorSketchy")));
                }
            }), offVisDH.map(function (d, i) {
                return ((0, jsx_runtime_1.jsx)(core_1.Tooltip, __assign({ title: d.reasoning }, { children: (0, jsx_runtime_1.jsx)(StyledComponents_1.DHIndicatorText, __assign({ x: findXPos(d), y: (bandScale(d.label) || 0) + 0.5 * bandScale.bandwidth() + (2 * Constants_1.IndicatorSize) * (i % 2 === 0 ? -1 : 1), fontSize: 'larger', isHighlighted: d.id === store.highlightedDH, onMouseOver: function () { store.setHighlightedDH(d.id); }, onMouseOut: function () { store.setHighlightedDH(-1); } }, { children: "* ".concat(d.content) }), "".concat(d.id, "-text")) }), void 0));
            }), exDH.map(function (d, i) {
                return ((0, jsx_runtime_1.jsx)(core_1.Tooltip, __assign({ title: d.reasoning }, { children: (0, jsx_runtime_1.jsx)(StyledComponents_1.DHIndicatorText, { isHighlighted: d.id === store.highlightedDH, fontSize: 'small' }, "".concat(d.id, "-text")) }), void 0));
            })] }, void 0));
};
