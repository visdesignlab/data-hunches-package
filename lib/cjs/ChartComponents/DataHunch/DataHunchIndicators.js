...
var DataHunchIndicator = function (_a) {
    ...
    return ((0, jsx_runtime_1.jsxs)("g", { children: [inVisDH.map(function (d, i) {
                ...
                if (inVisDH.length > 3) {
                    return ((0, jsx_runtime_1.jsx)(StyledComponents_1.DHIndicatorRect, { display: store.needToShowPreview ? 'none' : undefined, onMouseOver: function () { store.setSelectedDH(d.id); store.setHighlightedDH(d.id); }, x: honrizontalBandScale(d.label) || 0, width: honrizontalBandScale.bandwidth(), y: calculateY(d, true) }, d.id));
                }
                else {
                    return ((0, jsx_runtime_1.jsx)(SketchyBar_1.default, { dataHunch: d, xPos: (honrizontalBandScale(d.label) || 0) + (honrizontalBandScale.bandwidth() / inVisDH.length * i), yPos: calculateY(d, false), width: honrizontalBandScale.bandwidth() / inVisDH.length, height: calculateHeight(d) }, d.id));
                }
