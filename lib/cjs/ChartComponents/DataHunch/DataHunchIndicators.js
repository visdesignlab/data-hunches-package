...
var DataHunchIndicator = function (_a) {
    ...
    return ((0, jsx_runtime_1.jsxs)("g", { children: [inVisDH.map(function (d, i) {
                ...
                if (inVisDH.length > 3) {
                    return ((0, jsx_runtime_1.jsx)(DHIndicatorRect_1.default, { dataHunch: d, highlighted: d.id === store.highlightedDH || store.selectedDH.includes(d.id), xPos: calculateX(d, true), height: bandScale.bandwidth(), yPos: bandScale(d.label) || 0 }, "".concat(d.id, "-dhindicatorRect")));
                }
                else {
                    return ((0, jsx_runtime_1.jsx)(SketchyBar_1.default, { dataHunch: d, xPos: calculateX(d, false), yPos: (bandScale(d.label) || 0) + bandScale.bandwidth() / inVisDH.length * i, highlighted: d.id === store.highlightedDH || store.selectedDH.includes(d.id), width: calculateWidth(d), height: bandScale.bandwidth() / inVisDH.length }, "".concat(d.id, "-dhindicatorSketchy")));
                }
                ...
                return ((0, jsx_runtime_1.jsx)(core_1.Tooltip, __assign({ title: d.reasoning }, { children: (0, jsx_runtime_1.jsx)(StyledComponents_1.DHIndicatorText, __assign({ isHighlighted: d.id === store.highlightedDH || store.selectedDH.includes(d.id), x: Constants_1.IndicatorSize * (Math.floor(i / 2) + 1), y: (bandScale(d.label) || 0) + 0.5 * bandScale.bandwidth() + (2 * Constants_1.IndicatorSize) * (i % 2 === 0 ? -1 : 1), fontSize: 'small', onClick: function () { store.setSelectedDH([d.id]); } }, { children: "x" }), "".concat(d.id, "-text")) }), void 0));
            })] }, void 0));
};
