...
var DataHunchIndicator = function (_a) {
    ...
    return (_jsxs("g", { children: [inVisDH.map(function (d, i) {
                ...
                if (inVisDH.length > 3) {
                    return (_jsx(DHIndicatorRect, { display: store.needToShowPreview ? 'none' : undefined, onMouseOver: function () { store.setSelectedDH(d.id); store.setHighlightedDH(d.id); }, x: honrizontalBandScale(d.label) || 0, width: honrizontalBandScale.bandwidth(), y: calculateY(d, true) }, "".concat(d.id, "-dhindicatorRect")));
                }
                else {
                    return (_jsx(SketchyBar, { dataHunch: d, xPos: (honrizontalBandScale(d.label) || 0) + (honrizontalBandScale.bandwidth() / inVisDH.length * i), yPos: calculateY(d, false), width: honrizontalBandScale.bandwidth() / inVisDH.length, height: calculateHeight(d) }, "".concat(d.id, "-dhindicatorSketchy")));
                }
