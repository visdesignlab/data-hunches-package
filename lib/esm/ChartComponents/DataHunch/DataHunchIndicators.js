...
var DataHunchIndicator = function (_a) {
    ...
    return (_jsxs("g", { children: [inVisDH.map(function (d, i) {
                if (parseFloat(d.content) > verticalValueScale.domain()[0]) {
                    return _jsx(OverAxisIndicator, { dataHunch: d }, d.id);
                }
                if (inVisDH.length > 3) {
                    return (_jsx(DHIndicatorRect, { display: store.needToShowPreview ? 'none' : undefined, onMouseOver: function () { store.setSelectedDH(d.id); store.setHighlightedDH(d.id); }, x: honrizontalBandScale(d.label) || 0, width: honrizontalBandScale.bandwidth(), y: calculateY(d, true) }, d.id));
                }
                else {
                    return (_jsx(SketchyBar, { dataHunch: d, xPos: (honrizontalBandScale(d.label) || 0) + (honrizontalBandScale.bandwidth() / inVisDH.length * i), yPos: calculateY(d, false), width: honrizontalBandScale.bandwidth() / inVisDH.length, height: calculateHeight(d) }, d.id));
                }
            }), offVisDH.map(function (d, i) {
                return (_jsx(Tooltip, __assign({ title: d.reasoning }, { children: _jsx(DHIndicatorText, __assign({ x: (honrizontalBandScale(d.label) || 0) + 0.5 * honrizontalBandScale.bandwidth() + (2 * IndicatorSize + IndicatorSpace) * (i % 2 === 0 ? -1 : 1), y: store.svgHeight - margin.bottom + 25 + 2 * (IndicatorSize + IndicatorSpace) * Math.floor(i / 2), fontSize: d.type === 'exclusion' ? 'small' : 'large', isHighlighted: d.id === store.highlightedDH, onMouseOver: function () { store.setHighlightedDH(d.id); }, onMouseOut: function () { store.setHighlightedDH(-1); } }, { children: d.type === 'exclusion' ? 'x' : '*' }), d.id) }), d.id));
            })] }, void 0));
};
