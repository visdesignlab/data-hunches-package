...
var DataHunchIndicator = function (_a) {
    ...
    return (_jsxs("g", { children: [dataHunchDictionary.dataSpace.map(function (d, i) {
            ...
            }), dataHunchDictionary.cat.map(function (d, i) {
                var catWidth = (valueScale(dataPoint.value) - margin.left - 40) / DataPreset[store.dbTag].categories.length;
                return (_jsx(CategoricalIndicator, { highlighted: store.highlightedDH === d.id, selected: store.selectedDH.includes(d.id), xPos: valueScale(dataPoint.value) - 40 - (i + 1) * catWidth, yPos: (bandScale(dataPoint.label) || 0) + 3, width: catWidth, height: bandScale.bandwidth() - 6, dataHunch: d }, "cat-".concat(d.id)));
            })] }, void 0));
};
