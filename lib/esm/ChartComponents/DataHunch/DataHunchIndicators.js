...
var DataHunchIndicator = function (_a) {
    ...
    return (_jsxs("g", { children: [inVisDH.map(function (d, i) {
                ...
                if (inVisDH.length > 3) {
                    return (_jsx(DHIndicatorRect, { dataHunch: d, highlighted: d.id === store.highlightedDH || store.selectedDH.includes(d.id), xPos: calculateX(d, true), height: bandScale.bandwidth(), yPos: bandScale(d.label) || 0 }, "".concat(d.id, "-dhindicatorRect")));
                }
                else {
                    return (_jsx(SketchyBar, { dataHunch: d, xPos: calculateX(d, false), yPos: (bandScale(d.label) || 0) + bandScale.bandwidth() / inVisDH.length * i, highlighted: d.id === store.highlightedDH || store.selectedDH.includes(d.id), width: calculateWidth(d), height: bandScale.bandwidth() / inVisDH.length }, "".concat(d.id, "-dhindicatorSketchy")));
                }
            }), offVisDH.map(function (d, i) {
                return (_jsx(Tooltip, __assign({ title: d.reasoning }, { children: _jsx(DHIndicatorText, __assign({ x: findXPos(d, i, offVisDH.length), y: (bandScale(d.label) || 0) + 0.5 * bandScale.bandwidth() + (2 * IndicatorSize) * (i % 2 === 0 ? -1 : 1), fontSize: 'larger', isHighlighted: d.id === store.highlightedDH || store.selectedDH.includes(d.id), onClick: function () { store.setSelectedDH([d.id]); }, onMouseOver: function () { store.setHighlightedDH(d.id); }, onMouseOut: function () { store.setHighlightedDH(-1); } }, { children: calculateText(d.content, findXPos(d, i, offVisDH.length), offVisDH.length) }), "".concat(d.id, "-text")) }), void 0));
            }), exDH.map(function (d, i) {
                return (_jsx(Tooltip, __assign({ title: d.reasoning }, { children: _jsx(DHIndicatorText, __assign({ isHighlighted: d.id === store.highlightedDH || store.selectedDH.includes(d.id), x: IndicatorSize * (Math.floor(i / 2) + 1), y: (bandScale(d.label) || 0) + 0.5 * bandScale.bandwidth() + (2 * IndicatorSize) * (i % 2 === 0 ? -1 : 1), fontSize: 'small', onClick: function () { store.setSelectedDH([d.id]); } }, { children: "x" }), "".concat(d.id, "-text")) }), void 0));
            })] }, void 0));
};
