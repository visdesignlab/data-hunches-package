    ...
    return (dataHunchArray.length > 0 ?
        _jsx("g", __assign({ display: store.needToShowPreview ? 'none' : undefined }, { children: polygonPoints.map(function (d, i) {
                if (i < dataHunchArray.length) {
                    return _jsx(SketchyPolygon, { dataHunch: dataHunchArray[i], highlighted: dataHunchArray[i].id === store.highlightedDH || store.selectedDH.includes(dataHunchArray[i].id), points: d, opacity: chooseFill(i)[1], fill: chooseFill(i)[0].toString() }, "polygon-".concat(i));
                }
                else {
