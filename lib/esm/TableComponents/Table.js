...
var Table = function (_a) {
    ...
    var deleteDHHandler = function (e) {
        if (e.field === 'delete' && e.value === 'x') {
            ...
            deleteDoc(doc(firebaseSetup, store.datasetName, "sub".concat(store.currentVol), 'dhs', e.row.id.toString())).then(function (output) {
                console.log(output);
                store.setTotalDH(store.numOfDH - 1);
            }).catch(function (error) { console.log(error); });
        }
    };
    return (_jsx("div", __assign({ style: { height: '80vh', overflow: 'auto' } }, { children: _jsx(DataGrid, { className: styles.table, onRowEnter: function (d) { rowHoverHandler(d.row); }, onRowOut: rowOutHandler, columns: columns, checkboxSelection: true, onSelectionModelChange: function (d) { store.setSelectedDH(d); }, onCellClick: deleteDHHandler, rows: dhWithDelete }, void 0) }), void 0));
};
