...
export var handlePreviewOnClick = function (ogDataSet, labelToPreview, valueToPreview, svgHeight, SVGWidth, doesContainCategory, modelInput, categories) {
    var valueScale = makeValueScale(ogDataSet, SVGWidth);
    var bandScale = makeBandScale(ogDataSet, svgHeight);
    var categoricalScale = makeCategoricalScale(categories);
    ...
    select('#rectangles-preview')
        ...
        .join("rect")
        .attr('stroke-width', 4)
        .attr('stroke', function (d) { return d.label === labelToPreview ? SelectionColor : 'none'; })
        .attr('x', margin.left)
...
export var handleResetOnClick = function (ogDataSet, svgHeight, SVGWidth, doesContainCategory, selectedDP, categories) {
    var valueScale = makeValueScale(ogDataSet, SVGWidth);
    var bandScale = makeBandScale(ogDataSet, svgHeight);
    var categoricalScale = makeCategoricalScale(categories);
    select('#axis-mask')
    ...
    select('#rectangles-preview')
        ...
        .attr('stroke', function (d) { return d.label === selectedDP ? SelectionColor : 'none'; })
        .attr('x', margin.left)
        .attr('width', function (d) { return valueScale(d.value) - margin.left; })
        .attr('y', function (d) { return bandScale(d.label) || 0; })
        .attr('height', bandScale.bandwidth());
